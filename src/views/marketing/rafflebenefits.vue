      ...
      <div class="content">
        <div class="contanternews">
          ...
          <div class="dataGeneral bannerTable">
            <div class="flex">
              <div class="borderButton cursor"  @click="addCommodity()">新建0元抽奖</div>
              <div class="borderButton cursor marginLeft10"  @click="showhelprules = true">助力规则</div>
            </div>
            ...
            <table  v-if="noData" class="table">
              ...
              <tr v-for="(item,index) in tableData" :key="item.id" :inde="index">
                ...
                <td>
                  ...
                  <span class="line" v-if="(item.status != 4)"></span>
                  <span class="color2087 font12 cursor fontWeight" @click="enableGoods(item.id)" v-if="(item.status == 4)">使生效</span>
                  <span class="line"  v-if="(item.status == 4)"></span>
                  <span class="color2087 font12 cursor fontWeight" @click="copyGoods(item.id)" >复制</span>
                  <span class="line" ></span>
  ...
  export default {
    ...
    methods: {
      ...
      sortRecData(firstId,secondId){
        ...
        Service.lottery().sortactivity({},str).then(response => {
          if(response.errorCode == 0){
      ...
      enableGoods(id){
        this.$confirm('确认使活动生效?', '', {
          ...
          Service.lottery().invalidactivity({
          },id).then(response => {
            if(response.errorCode == 0){
              this.$message({
                type: 'success',
      ...
      delectCommodity(id){
        this.$confirm( '活动删除请谨慎操作，确定删除?', '', {
          ...
          Service.lottery().deleteactivity({
          },id).then(response => {
            if(response.errorCode == 0){
              this.$message({
                type: 'success',
      ...
      addCommodity(){
        this.$router.push({name:'addrafflebenefits'})
      },
      goDetail(id){
        this.$router.push({name:'addrafflebenefits',query:{id:id}})
      },
      ...
      getcommodityData(str){
        ...
        let paramsKey = this.activeIndex == 0 ? '' : this.activeIndex
        Service.lottery().queryactivity({
        }, paramsKey).then(response => {
          if(response.errorCode == 0){
            if(response.data.length == 0){
              ...
              for(var i = 0;i< response.data.length;i++){
                response.data[i].isshowDes = false;
              }
              this.$nextTick(()=>{
