        ...
        <div class="dataGeneral backWhite">
          ...
          <div class="searchContent itemscenter  flexBetween">
            <div class="channnelTitle">
              <span class="colorGrey font12 marginright10">已上架商品（除积分兑换商品外）</span>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      getGoods() {
        ...
        if (goodsDatas.length > 1 && (this.source == 'addrafflebenefits' || this.source == 'addactiveh5')) {
          this.$message.error('仅可选择1个商品')
          return
        }
        if (this.source == 'addrafflebenefits' || this.source == 'addactiveh5') {
          this.$emit('selectGuige', goodsDatas[0]);
        } else if (this.source == 'commoditygroup') {
